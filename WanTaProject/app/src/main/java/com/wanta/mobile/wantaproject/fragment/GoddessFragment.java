 ...
 */
public class GoddessFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {

    private View mView_goddess;
    private SwipeRefreshLayout goddess_fresh;
    private RecyclerView goddess_recycleview;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mView_goddess = inflater.inflate(R.layout.fragment_goddess, null);
        //加载信息
        initId();
        return mView_goddess;
    }

    private void initId() {
        goddess_fresh = (SwipeRefreshLayout) mView_goddess.findViewById(R.id.goddess_fresh);
        goddess_recycleview = (RecyclerView) mView_goddess.findViewById(R.id.goddess_recycleview);
        goddess_fresh.setOnRefreshListener(this);
//        //改变加载显示的颜色
        goddess_fresh.setColorSchemeResources(R.color.mostpopular_fresh_color);
        //放在子线程中是因为mostpopular_fresh还没有初始化完，加载的图标是不会显示的
        goddess_fresh.post(new Runnable() {
            @Override
            public void run() {
                onRefresh();
            }
    ...
    private void init() {
        ...
        goddess_recycleview.setLayoutManager(staggeredGridLayoutManager);
        mAdapter = new MostPopularFragmentAdapter(getActivity(), all_message);
        goddess_recycleview.setAdapter(mAdapter);
        goddess_recycleview.setOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
...
//                ToastUtil.showShort(getActivity(),"作者");
        ...
        mAdapter.setOnMostPopularItemHeartListener(new MostPopularFragmentAdapter.onMostPopularItemHeartListener() {
            ...
            public void onItemHeartClick(View view, final CustomSimpleDraweeView item_mostpopular_heart, final MostPopularInfo mostPopularInfo, final TextView textView) {
                final int position = goddess_recycleview.getChildAdapterPosition(view);
    ...
    class MostPopularAsyTask extends AsyncTask<String, Integer, String> {
        ...
        protected void onPreExecute() {
            goddess_fresh.setRefreshing(true);
        }
        ...
        protected void onPostExecute(String s) {
            goddess_recycleview.scrollToPosition(0);
            goddess_fresh.setRefreshing(false);
        }
    }
