...
public class NewestFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private View mView_newes;
    private SwipeRefreshLayout newest_fresh;
    private RecyclerView newest_recycleview;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mView_newes = inflater.inflate(R.layout.fragment_newest, null);
       initId();
        return mView_newes;
    }
    private void initId() {
        newest_fresh = (SwipeRefreshLayout) mView_newes.findViewById(R.id.newest_fresh);
        newest_recycleview = (RecyclerView) mView_newes.findViewById(R.id.newest_recycleview);
        newest_fresh.setOnRefreshListener(this);
//        //改变加载显示的颜色
        newest_fresh.setColorSchemeResources(R.color.mostpopular_fresh_color);
        //放在子线程中是因为mostpopular_fresh还没有初始化完，加载的图标是不会显示的
        newest_fresh.post(new Runnable() {
            @Override
            public void run() {
                onRefresh();
            }
    ...
    private void init() {
        ...
        newest_recycleview.setLayoutManager(staggeredGridLayoutManager);
        mAdapter = new MostPopularFragmentAdapter(getActivity(), all_message);
        newest_recycleview.setAdapter(mAdapter);
        newest_recycleview.setOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
...
//                ToastUtil.showShort(getActivity(),"作者");
        ...
        mAdapter.setOnMostPopularItemHeartListener(new MostPopularFragmentAdapter.onMostPopularItemHeartListener() {
            ...
            public void onItemHeartClick(View view, final CustomSimpleDraweeView item_mostpopular_heart, final MostPopularInfo mostPopularInfo, final TextView textView) {
                final int position = newest_recycleview.getChildAdapterPosition(view);
    ...
    class MostPopularAsyTask extends AsyncTask<String, Integer, String> {
        ...
        protected void onPreExecute() {
            newest_fresh.setRefreshing(true);
        }
        ...
        protected void onPostExecute(String s) {
            newest_recycleview.scrollToPosition(0);
            newest_fresh.setRefreshing(false);
        }
    }
