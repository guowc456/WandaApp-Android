package com.wanta.mobile.wantaproject.calendar;
...
public class MonthDateView extends View {
	...
	private int mSelectBGColor = Color.parseColor("#790887");
	...
	private int mCircleColor = Color.parseColor("#CE93E7");
	private List<Integer> daysHasThingList;
//	private int position = 1;//标注当前选择的个数
	public MonthDateView(Context context, AttributeSet attrs) {
		super(context, attrs);
		mDisplayMetrics = getResources().getDisplayMetrics();
		Calendar calendar = Calendar.getInstance();
		mPaint = new Paint();
		mPaint.setAntiAlias(true);//消除锯齿状
	...
	protected void onDraw(Canvas canvas) {
		...
		String dayString;
		int mMonthDays = CalendarDateUtils.getMonthDays(mSelYear, mSelMonth);
		int weekNumber = CalendarDateUtils.getFirstDayWeek(mSelYear, mSelMonth);
...
//				canvas.drawRect(startRecX, startRecY, endRecX, endRecY, mPaint);
				canvas.drawCircle(startRecX+mColumnSize/2,startRecY+mRowSize/2,mCircleRadius,mPaint);
			...
			if(dayString.equals(mCurrDay+"")){
				...
				canvas.drawText("今",startRecX+mColumnSize/3+3,startRecY+mRowSize-22,mPaint);
			}
		}
	}
