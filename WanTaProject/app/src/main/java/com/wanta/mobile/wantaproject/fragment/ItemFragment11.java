    ...
    private void initData() {
        ...
        adapter.setOnItemClickListener(new ItemTagsDetailRecycleViewAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View v, MyImageView tags_detail_image) {
                ...
                if (Constants.TAGS_SELECT_IAMGE_URL5.contains(Constants.TAGS_IAMGE_URL.get(position))){
                    //点击之后取消
                    Constants.TAGS_SELECT_IAMGE_URL5.remove(Constants.TAGS_IAMGE_URL.get(position));
                    tags_detail_image.setVisibility(View.GONE);
                    JsonParseUtils.getToString(Constants.TAGS_SELECT_IAMGE_URL5);
                }else {
                    Constants.TAGS_SELECT_IAMGE_URL5.add(Constants.TAGS_IAMGE_URL.get(position));
