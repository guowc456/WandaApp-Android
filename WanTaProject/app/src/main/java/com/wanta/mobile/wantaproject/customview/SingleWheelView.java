 ...
 * Date: 7/1/14.
 */
public class SingleWheelView extends ScrollView {
    public static final String TAG = SingleWheelView.class.getSimpleName();
                ...
                if (initialY - newY == 0) { // stopped
                    ...
                    if (remainder == 0) {
                        ...
                        if (remainder > itemHeight / 2) {
                            SingleWheelView.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    SingleWheelView.this.smoothScrollTo(0, initialY - remainder + itemHeight);
                            ...
                            SingleWheelView.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    SingleWheelView.this.smoothScrollTo(0, initialY - remainder);
                                    selectedIndex = divided + offset;
                                    onSeletedCallBack();
                                }
                            });
                    ...
                    initialY = getScrollY();
                    SingleWheelView.this.postDelayed(scrollerTask, newCheck);
                }
            }
        };
...
//            position = divided + offset + 1;
        ...
        for (int i = 0; i < childSize; i++) {
            ...
            if (position == i) {
                ...
                itemView.setTextColor(Color.parseColor("#e1dbdb"));
            }
        }
    }
    ...
    public void setBackgroundDrawable(Drawable background) {
        ...
        if (null == paint) {
            paint = new Paint();
            paint.setColor(Color.parseColor("#e1dbdb"));
...
//                canvas.drawLine(0, obtainSelectedAreaBorder()[1], viewWidth , obtainSelectedAreaBorder()[1], paint);
    ...
    public void setSeletion(int position) {
        ...
        this.post(new Runnable() {
            @Override
            public void run() {
                SingleWheelView.this.smoothScrollTo(0, p * itemHeight);
            }
        });
